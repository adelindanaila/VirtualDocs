...
export default {
    ...
    methods: {
        ...
        remove( index, files, alternative_index = -1 ) {
            ...
            const element = document.querySelector(`#dropzone_wrapper ul li[data-index="${element_index}"]`)
            ...
            elements.forEach(( node ) => {
                ...
                inputs.forEach(( snap, index ) => {
                    ...
                    if( !input.value.length )
                        this.error_add_classes( label, input, select )

                    else
                        _values[`input${index}`] = input.value

                })

                if( !_.isEmpty( _values ) && _.size(_values) === inputs.length )
                    values.push(_values)
