<template>
  <div class="fixed z-10 inset-0 overflow-y-auto">
    <div
      id="manage_categories_modal_wrapper"
    ...
    >
      ...
      <div
        id="manage_categories_modal_content"
        ...
        >
          ...
          >
            ...
            >
              ...
              <div>
                <div v-if="!category.delete" class="flex items-center">
                  <svg
                    v-if="category.edit"
                    @click="edit(category._id, index)"
                    class="w-5 h-5 cursor-pointer stroke-current transition hover:text-indigo-400 select-none"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg"
...
export default {
  ...
  methods: {
    ...
    async edit(id, index) {
      ...
      if (!input.value.length) this.error_add_classes(input)
      else {
        try {
    ...
    error_checker(event) {
      ...
      if (input.classList.contains('border-red-700')) this.error_remove_classes(input)
      else {
        if (!input.value.length) this.error_add_classes(input)
      }
    },
