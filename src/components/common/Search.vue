<template>
    <div class="relative text-gray-600 mx-auto w-3/4">
        <input
            ...
            type="search" name="serch" placeholder="Search document" class="text-base rounded bg-white shadow w-full h-10 px-5 pr-10 text-sm focus:outline-none">
        ...
        <div v-if="documents.length" id="hero_search_results" class="bg-white shadow absolute w-full z-10" style="top: calc(40px + 1rem);">
            <div
                v-for="document in documents" :key="document._id"
                ...
                <div class="text-left ml-4">
                    <div class="text-sm font-medium text-gray-90">{{ document.name }}</div>
    ...
    methods: {
        ...
        async search( ) {
            ...
            try {
                ...
                const params = new URLSearchParams({ name: this.name, limit: this.search_limit }).toString( )
