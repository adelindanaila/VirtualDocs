<template>
  <div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div
      id="signin_form_wrapper"
      class="max-w-md w-full space-y-8 animate__animated animate__faster animate__fadeIn"
    >
      <div>
        <img
  ...
  methods: {
    async submit(e) {
      ...
      try {
        ...
        if (errors.email === 'email_missing') alert('email_missing')

        if (errors.email === 'email_invalid') alert('email_invalid')

        if (errors.email === 'email_incorrect') alert('email_incorrect')

        if (errors.password === 'password_missing') alert('password_missing')

        if (errors.password === 'password_length') alert('password_length')

        if (errors.password === 'password_incorrect') alert('password_incorrect')
      }
    },
  },
}
