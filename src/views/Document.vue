<template>
  ...
  <div
    v-if="!loading"
    class="bg-white shadow-sm animate__animated animate__fadeIn animate__faster py-8"
  >
  ...
  <div id="document_content" class="max-w-7xl mx-auto">
    <div
      v-if="loading"
      id="loader_wrapper"
      class="w-full flex items-center justify-center animate__animated animate__fadeIn animate__faster"
    >
      <div class="lds-ring">
        <div></div>
        <div></div>
  ...
  async mounted() {
    ...
    try {
      ...
      const task = pdfjsLib.getDocument(
        `${this.api_url}/${this.upload_folder}/${response.data.file}.pdf`
      )
